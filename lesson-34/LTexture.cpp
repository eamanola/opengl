...
void LTexture::createPixels( GLuint imgWidth, GLuint imgHeight, GLuint pixelFormat )
{
  if( imgWidth > 0 && imgHeight > 0 )
  {
    ...
    {
      memset( getPixelData( pixelFormat ), 0, size );
    }
...
void LTexture::copyPixels( void* pixels, GLuint imgWidth, GLuint imgHeight, GLuint pixelFormat )
{
  if( imgWidth > 0 && imgHeight > 0 )
  {
    freeTexture();
...
{
  return loadPixelsFromFile32(path) && loadTextureFromPixels32();
}
